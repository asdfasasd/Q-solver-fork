...
// ForceTopmost 强制置顶（不改样式，仅更新 Z 顺序，供定时器调用）
func ForceTopmost(hwnd WindowHandle) {
...
// verifyWindowDisplayAffinity 验证防录屏设置是否已生效
func verifyWindowDisplayAffinity(hwnd uintptr, expected uint32) bool {
	var affinity uint32
	ret, _, _ := procGetWindowDisplayAffinity.Call(hwnd, uintptr(unsafe.Pointer(&affinity)))
	return ret != 0 && affinity == expected
}
...
// getHwndByPid 根据进程ID获取窗口句柄（仅返回可见的顶层窗口）
...
func applyGhostMode(hwnd uintptr) {
	...
	// 优先尝试"隐身(透明)"，如果失败降级为"黑屏"；尝试最多3次
	applied := false
	for attempt := 0; attempt < 3; attempt++ {
		err := setWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE)
		if err == nil && verifyWindowDisplayAffinity(hwnd, WDA_EXCLUDEFROMCAPTURE) {
	...
	// SWP_NOZORDER 去掉，改用 HWND_TOPMOST 真正置顶；保留 SWP_FRAMECHANGED 让去边框生效
	SetWindowPos(hwnd, HWND_TOPMOST, 0, 0, 0, 0,
		uint32(SWP_NOMOVE|SWP_NOSIZE|SWP_FRAMECHANGED|SWP_NOACTIVATE))
}
