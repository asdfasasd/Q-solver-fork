...
> [!NOTE]
> **📌 原项目声明**：本项目 Fork 自 [whale-coding/Q-solver](https://github.com/whale-coding/Q-solver)，遵循原项目 [CC BY-NC 4.0](LICENSE) 协议。在原版基础上新增了以下改进：防截图/录屏修复（启动即生效）、管理员权限置顶对抗考试软件、200ms 心跳抢回置顶、顶栏隐身状态指示。
...
| 特性 | 描述 |
|:---|:---|
| **🚫 防录屏检测** | 窗口对大多数录屏/截屏软件不可见；启动时立即生效，顶栏「隐藏/展示」按钮点亮表示防护已开启 |
| **👻 鼠标穿透** | 开启后可透过窗口点击后方内容，互不影响 |
| **📌 强力置顶** | 以管理员权限运行，每 200ms 主动抢回置顶，可对抗考试类软件的焦点抢占 |
| **🔕 沉浸免打扰** | 精心设计的焦点管理，输入时不抢占主窗口焦点 |

> 💡 **考试软件兼容**：应用以管理员权限（High Integrity）启动，快捷键不会被考试软件的键盘钩子拦截。
...
**环境要求**：Go 1.25+, Node.js 22+, Wails CLI, GCC（Windows 需要 MSYS2 MinGW）
...
# 3. 开发模式运行（需以管理员身份执行，支持热重载）
wails dev

# 4. 编译发布版本（同时生成 exe 和 NSIS 安装包）
wails build -nsis
```

> [!NOTE]
> **Windows 构建提示**：需要安装 [MSYS2](https://www.msys2.org/) 并通过 `pacman -S mingw-w64-x86_64-gcc` 安装 GCC，构建时将 `C:\msys64\mingw64\bin` 加入 PATH。由于 manifest 声明了管理员权限，`wails dev` 本身也需要在管理员终端中运行。
